...
function mod.configure_picker(picker)
  ...
  elseif picker == mod.supported_pickers.snacks then
    ...
    vim.keymap.set('n', '<leader>s/', Snacks.picker.grep, { desc = 'Search by grep' })
    vim.keymap.set('n', '<leader>sg', Snacks.picker.lines, { desc = 'Search by grep in current buffer' })
    vim.keymap.set('n', '<leader>sG', Snacks.picker.grep_buffers, { desc = 'Search by grep in open buffers' })
    -- vim.keymap.set('n', '<leader>sG', ':LiveGrepGitRoot<cr>', { desc = 'Search by grep on git root' })
